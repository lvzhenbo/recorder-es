...
- 🔄 **完整控制** - 开始、停止、暂停和恢复录音
- 🎵 **格式转换** - 基于 mediabunny 支持转换为 MP4、MOV、MKV、WebM、OGG、MP3<sup>*</sup>、WAV、AAC、FLAC 等格式
...
> <sup>*</sup> MP3 格式需要额外安装 `@mediabunny/mp3-encoder` 扩展，详见[浏览器兼容性](#浏览器兼容性)
...
根据 [mediabunny 文档](https://mediabunny.dev/guide/output-formats#mp3)，MP3 格式有特殊要求：

1. **编码器依赖**：MP3 编码需要使用 `mp3-encoder` 扩展
   ```bash
...
2. **使用方式**：
   ...
   // 确保已安装 @mediabunny/mp3-encoder
   const mp3Blob = await convertAudio(webmBlob, {
     format: 'mp3',
     audioBitsPerSecond: 192000, // 可选，默认128kbps
   });
   ```
...
3. **注意事项**：
   - MP3 仅支持音频轨道（无视频）
   - 仅允许一个音频轨道
   - 需要 PCM 音频输入（通常为 16 位采样）
   - 如果未安装 `@mediabunny/mp3-encoder`，转换为 MP3 将失败

如果不需要 MP3 格式，可以使用其他格式如 WebM、OGG 或 AAC，它们不需要额外的编码器扩展。
